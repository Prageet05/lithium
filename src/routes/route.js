...
[
   ...
   router.post("/players", function(req, res) {
        let substitute= req.body
        let present=req.body.name
        for(elements of players){
            if(elements.name==present){
...
let persons= [
 ...
 router.post("/persons", function(req, res) {
     let present=req.query.votingAge
    let arr3 =[]
    persons.forEach((persons)=>{
        if(persons.age>present){
            persons.votingStatus=true
            arr3.push(persons)
        }
    })
    // persons.push(substitute)
res.send(  { Neta_k_kaam_ke: arr3 }  )
})
